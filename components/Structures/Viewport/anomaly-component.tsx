...
export function AnomalyComponent({ anomaly, onClick, isHighlighted = false }: AnomalyComponentProps) {
  ...
  const getShapeStyles = () => {
    const commonStyles = {
      ...
      background: `radial-gradient(circle, ${anomaly.color}CC 0%, ${anomaly.color}88 60%, ${anomaly.color}44 100%)`,
      boxShadow: isHighlighted
        ? `
          0 0 ${30 * size}px ${15 * size}px ${anomaly.color}66, 
          0 0 ${50 * size}px ${25 * size}px ${anomaly.color}33,
          0 0 ${70 * size}px ${35 * size}px rgba(168,216,234,0.2)
        `
        : `
          0 0 ${20 * size * anomaly.glowIntensity}px ${10 * size * anomaly.glowIntensity}px ${anomaly.color}44,
          0 0 ${30 * size * anomaly.glowIntensity}px ${15 * size * anomaly.glowIntensity}px ${anomaly.color}22
        `,
    ...
    switch (anomaly.shape) {
      case "circle":
        return {
          ...commonStyles,
          borderRadius: "50%",
          border: `1px solid ${anomaly.color}88`,
        }
      case "star":
        return {
          ...commonStyles,
    ...
    <>
      ...
      >
        ...
        <div
          ...
          style={{
            background: `radial-gradient(circle, ${anomaly.color}AA 0%, ${anomaly.color}44 50%, transparent 80%)`,
