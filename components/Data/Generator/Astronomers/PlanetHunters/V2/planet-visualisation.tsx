...
export function PlanetVisualization({ stats }: PlanetVisualizationProps) {
  ...
  useEffect(() => {
    ...
    for (let y = 0; y < resolution; y++) {
      for (let x = 0; x < resolution; x++) {
        ...
        features.push({
          ...
          properties: {
            elevation,
            color: getColor(elevation, stats.density, stats.type || "terrestrial"),
          },
        })
      }
    }
    ...
    if (stats.type === "gaseous") {
      ...
      atmosphere.append("stop").attr("offset", "85%").attr("stop-color", "#4B9CD3").attr("stop-opacity", 0.15)
  ...
  function getColor(elevation: number, density: number, type: "terrestrial" | "gaseous"): string {
    if (type === "gaseous") {
      return density > 1.5
        ? `rgb(${255 * elevation}, ${165 * elevation}, ${0})`
        : `rgb(${183 * elevation}, ${226 * elevation}, ${242 * elevation})`
    } else {
