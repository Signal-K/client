...
const NewUserModal = (): JSX.Element => { /* export default function OffchainAccount({ session }: { session: Session}) { */
    ...
    const [avatar_url, setAvatarUrl] = useState(''); // Might be called `imageUrl` in this dir -> keep an eye out for this. `avatar_url` is what is in the `profiles` table on the database. This is the way
    const [address, setAddress] = useState('');
    /* const [username, setUsername] = useState<Profiles['username']>(null); --> Update above fields to this <> structure to match up with the database types
    ...
    async function handleSubmit(e: React.ChangeEvent<HTMLFormElement>): Promise<void> {
        ...
        if (error) {
            ...
            return setFullNameError('Something went wrong. Please try again');
        }
