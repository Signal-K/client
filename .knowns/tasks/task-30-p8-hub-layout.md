...
- [x] Structure cards are tappable → navigate to that structure's viewport directly
...
- [x] Show user's deployed structures as positioned icons with labels
- [x] Structures animate: satellite orbits, rover moves, telescope sweeps (CSS only)
- [x] Tap structure → navigate to viewport or deploy route
- [ ] Zoom in → secondary entity detail view
- [x] Hide until at least 1 structure deployed
- [x] Empty state: "What do you want to do today?" prompt with clickable entity options
- [x] Show deployed structure state (standby = dim, active = glowing) on map icons

### Persistent HUD Strip
- [x] Create slim HUD strip component (top or bottom of hub, always visible)
- [x] Data: active signals count (`linked_anomalies` query), pending anomalies, (TBD: other metrics)
- [x] Numbers count up (odometer animation) when values change
- [x] Keep to 2–3 metrics max — no clutter

### Add Project Button
- [x] "Add project" button on hub (accessible at any time post-onboarding)
- [x] Opens project selection view (same component as onboarding step 1)
- [x] Does not restart onboarding — just opens project picker
...
### Leaderboard
- [x] Add leaderboard component to hub (placement TBD during design)
- [x] Primary metric TBD: classification count or stardust
- [x] Show top N + current user's rank
