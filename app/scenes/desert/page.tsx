...
const Bolt = ({ className = "" }: { className?: string }): ReactElement => (
  <div className={`w-3 h-3 rounded-full bg-muted-foreground/50 border border-muted-foreground/70 ${className}`}></div>
)

const BoltedCard = ({ children, className = "" }: BoltedCardProps): ReactElement => (
...
const MetricItem = ({ icon: Icon, label, value }: MetricItemProps): ReactElement => (
...
const StatItem = ({ label, value }: StatItemProps): ReactElement => (
