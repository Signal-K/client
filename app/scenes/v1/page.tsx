...
import {
  AllAutomatons,
  SingleAutomaton,
} from "@/app/components/_[archive]/Gameplay/Inventory/Automatons/Automaton";
  ...
  const fetchMissionCompletionStatus = async () => {
    if (session) {
      try {
        ...
        console.error(
          "Error fetching mission completion status:",
          error.message
        );
      }
    }
  ...
  async function fetchUserUtilityStructures() {
    if (session && activePlanet) {
      try {
        ...
        if (error) {
          console.error(
            "Error fetching user utility structures: ",
            error.message
          );
          return;
        }
          ...
          <div className="flex flex-col flex-1 overflow-hidden">
            <div className="relative flex-1 overflow-hidden bg-primary-foreground transition-all duration-300">
              ...
              <div className="flex h-full items-center justify-center p-8">
                <div className="space-y-4 text-center">{renderContent()}</div>
              </div>
            </div>
