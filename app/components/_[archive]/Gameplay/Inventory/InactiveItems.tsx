...
const UserItemsUndeployed = () => {
    const supabase = useSupabaseClient();
    const session = useSession();
    const { activePlanet } = useActivePlanet();
    const [inventoryItems, setInventoryItems] = useState<InventoryEntry[] | null>(null);
    const { triggerRefresh } = useRefresh(); // Use the triggerRefresh function from context
        ...
        try {
            if (session && activePlanet) {
                ...
                if (data) {
                    ...
                    triggerRefresh(); // Trigger refresh to re-render AllStructures component
                };
            };
        } catch (error: any) {
...
export default UserItemsUndeployed;
