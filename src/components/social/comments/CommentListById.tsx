...
export function SurveyorCommentList({
    ...
    const fetchComments = async () => {
        ...
        return (
            <div className="w-full">
                <h3 className="text-lg font-semibold text-[#2E3440] mb-4">Community Surveys</h3>
                {loading ? (
                    <p className="text-[#4C566A]">Loading comments...</p>
                ) : comments.length === 0 ? (
                    <p className="text-[#4C566A]">No surveys have been completed yet</p>
                ) : (
                    <div className="space-y-4">
                        {comments.map(( comment ) => (
                            <Card key={comment.id} className="border border-[#D8DEE9] bg-white rounded-lg shadow-sm">
                                <CardContent className="p-4">
                                    <div className="flex items-start space-x-3">
                                        ...
                                        <div>
                                            <p className="text-sm font-medium text-[#2E3440]">{comment.author}</p>
                                            <p className="text-sm text-blue-300">{comment.category}: {comment?.value}</p>
                                            <p className="text-sm text-[#AC566A] whitespace-pre-line">{comment.content}</p>
                                            <p className="text-xs text-[#A3BE8C] mt-1">
                                                {new Date(comment.created_at).toLocaleString()}
                                            </p>
                                        </div>
