...
function getWeekDateRange(startDateStr: string): {
  start: string;
  end: string;
} {
...
export default function ClientClassificationPage({ id }: Props) {
  ...
  const nextImage = () => {
    setCurrentImageIndex(
      (prevIndex) => (prevIndex + 1) % diskDetectiveImages.length
    );
  };
  ...
  const checkMilestones = async (
    ...
    try {
      ...
      const matchingWeek = allMilestones.playerMilestones.find((week: any) => {
        ...
        return (
          new Date(createdAt) >= new Date(start) &&
          new Date(createdAt) < new Date(end)
        );
      });
  ...
  if (loading)
    return <div className="text-center mt-10 text-[#2E3440]">Loading...</div>;
  if (!classification) return null;

  const mediaUrl = Array.isArray(classification.media)
    ? classification.media.find(
        (item: any) =>
          Array.isArray(item) &&
  ...
  const annotationCounts = annotationOptions.reduce(
  ...
  const annotationBadges = Object.entries(annotationCounts).map(
    ([label, count], idx) => {
      ...
      <div className="flex items-center justify-center min-h-screen px-2 py-4 sm:px-4 sm:py-8">
        <div className="bg-[#F8FAFC] rounded-2xl shadow-xl p-3 sm:p-6 max-w-4xl w-full border border-[#88C0D0] space-y-4 sm:space-y-6 max-h-[95vh] overflow-y-auto">
                ...
                {classification.classificationConfiguration.interpretation && (
                  <div
                    className={`p-3 sm:p-4 rounded-lg border ${
                      classification.classificationConfiguration.interpretation.category.includes(
                        "Disk Candidate"
                      ) ||
                      classification.classificationConfiguration.interpretation.category.includes(
                        "Planetary System"
                      ...
                      classification.classificationConfiguration.selectedOptions
                        ...
                        <div className="bg-blue-50 border border-blue-200 rounded-lg p-3">
                          ...
                          <ul className="space-y-1">
                            {classification.classificationConfiguration.selectedOptions.map(
          ...
          <div className="space-y-3 pt-2">
            <h2 className="text-md font-semibold text-[#2E3440]">
              🎯 Milestones
            </h2>
                  ...
                  <div className="pt-1">
                    {m.achieved ? (
                      ...
                      <>
                        ...
                        <Progress
                          value={(m.currentCount / m.requiredCount) * 100}
                        />
                      </>
                    )}
                  </div>
          ...
          <div className="flex flex-col sm:flex-row gap-2 sm:gap-3 pt-3 sm:pt-4 border-t border-[#D8DEE9]">
            <Button
              variant="outline"
              onClick={() => router.push(viewportUrl)}
            ...
            {type === "planet" && (
              <Button
                variant="default"
                onClick={() =>
                  router.push(`/planets/paint/${classification.id}`)
                }
                className="text-xs sm:text-sm"
              >
                🪐 Customise planet
