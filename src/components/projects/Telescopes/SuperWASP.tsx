...
export function TelescopeSuperWASP() {
  const supabase = useSupabaseClient();
  const session = useSession();
  const { activePlanet } = useActivePlanet();
  ...
  const [hasMission, setHasMission] = useState<boolean | null>(null);
  ...
  useEffect(() => {
    async function fetchAnomaly() {
      ...
      try {
        // First, try to find a linked anomaly for this user
        ...
        // Filter for superwasp anomalies
        const superwaspLinked = linkedData?.find((link: any) => 
          link.anomalies?.anomalySet === "superwasp-variable" || 
          link.anomalies?.anomalySet === "telescope-superwasp-variable"
        );

        if (superwaspLinked?.anomalies) {
          const anomalyData = superwaspLinked.anomalies as any;
          setAnomaly({
            id: anomalyData.id?.toString() || '',
            name: anomalyData.content || '',
            content: anomalyData.content || '',
          });
          const supabaseUrl = process.env.NEXT_PUBLIC_SUPABASE_URL;
          setImageUrl(`${supabaseUrl}/storage/v1/object/public/telescope/telescope-superwasp-variable/${anomalyData.id}.gif`);
        } else {
          // Fallback: check for anomalies authored by user
          const { data: anomalyData, error: anomalyError } = await supabase
            .from("anomalies")
            .select("*")
            .eq("author", session.user.id)
            .eq("anomalySet", "superwasp-variable")
            .limit(1)
            .maybeSingle();
  ...
  useEffect(() => {
    const checkMission = async () => {
      ...
      // For now, allow access if user has any linked SuperWASP anomaly
      // The mission check can be added later if needed
      setHasMission(true);
      setMissionLoading(false);
      
      /* Original mission check - commented out for now
      try {
  ...
  if (!hasMission) {
    return (
      <div className="text-center p-4">
        <p>You need to unlock this mission first. Complete the required tasks to access the SuperWASP variable star classification.</p>
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-col items-start gap-4 pb-4 relative w-full overflow-y-auto max-h-[90vh] rounded-lg overflow-x-hidden">
      ...
      {showTutorial && <SuperWASPTutorial anomalyId={anomaly.id} />}

      {imageUrl && !showTutorial && (
        <div className="flex flex-col items-center w-full">
          <ClassificationForm
            anomalyId={anomaly.id}
            anomalyType="SuperWASP"
